...
def main():
    ...
    for i in range(1,27):
        ...
        with open(main_wd+'LINKPHASE/chr_'+str(i)+'/recombinations_hmm','r') as f:
            for ligne in f:
                buf = ligne.split()
                ## Prefix individuals' names with population
                buf[0]='Soay_'+buf[0]
                buf[1]='Soay_'+buf[1]
                try:
                    lepere = pere[buf[1]]
    ...
    with open(main_wd+"FIDs.txt") as f:
        for i,ligne in enumerate(f):
            ...
            buf[0]='Soay_'+buf[0]
            FIDs[buf[0]]=1
    physmap_soay=physmap
    physmap=[]
    ### Read in Lacaune Data
    main_wd='data/family/'
    for i in range(1,27):
        ## get marker information
        carte_chr={} 
        ...
        with open(main_wd+'LINKPHASE/chr_'+str(i)+'/recombinations_hmm','r') as f:
            for ligne in f:
                buf = ligne.split()
                ## Prefix individuals' names with population
                buf[0]='Lacaune_'+buf[0]
                buf[1]='Lacaune_'+buf[1]
                try:
                    lepere = pere[buf[1]]
                except KeyError:
                    lepere = Parent(buf[1]) 
    ...
    with open(main_wd+"FIDs.txt") as f:
        for i,ligne in enumerate(f):
            ...
            buf[0]='Lacaune_'+buf[0]
            FIDs[buf[0]]=1
    ## Get everybody's phenotype
    Ribar=0
    S_Ri_mi=0
    with open("results/family/Soay_Lacaune_parent_recombination.txt",'w') as fout:
    ...
    if calibrate_prior:
        ...
        plt.savefig('results/family/soay_lacaune_Prior_cj.pdf')
        plt.close()
    else:
        cj_alpha=2.7
        cj_beta=2.0
    ...
    ## 1Mb map
    with open("results/family/soay_lacaune_1Mb_map.txt",'w') as fout:
        ...
        for i in range(26):
            ...
            for w_left in range(0,xlim,int(w_size_rec)):
                ...
                fout.flush()
    ## 60K intervals map
    with open("results/family/soay_lacaune_SNP_array_map.txt",'w') as fout,\
         open("results/family/SNP_array_map.txt") as flac:
        ...
        for i,ligne in enumerate(flac):
            ...
            sys.stdout.write("%d: %7d-%7d \r"%(chrom,w_left,w_right))
            sys.stdout.flush()
