...
basedir=`pwd`
datadir='data/Soay/'
tmpdir='data/Soay/linkphase.in'
plink_prefix='Soays_autosomes_clean'
mkdir -p $tmpdir

#Ped file creation

awk '{print $2, $3, $4}' $datadir/$plink_prefix.fam > $tmpdir/ped
...
do
    ...
    plink --sheep --bfile $datadir/$plink_prefix --chr $chr --recode 12 --out $wdir/typage
...
mkdir -p data/Soay/LINKPHASE
rsync -av --include="*/" --include="map" --include="recombinations_hmm" --include="ped" --exclude="*" data/Soay/linkphase.in/ data/Soay/LINKPHASE/
