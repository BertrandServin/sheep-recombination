...
def main():
    ...
    with open(main_wd+"FIDs.txt") as f:
        for i,ligne in enumerate(f):
            buf=ligne.split()
            FIDs[buf[0]]=1
    with open("results/Soay/parent_recombination.txt",'w') as fout:
    ...
    if calibrate_prior:
        ...
        plt.savefig('results/Soay/Prior_cj.pdf')
        plt.close()
    else:
        cj_alpha=2.7
        cj_beta=2.0
    ...
    ## 1Mb map
    with open("results/Soay/1Mb_map.txt",'w') as fout,\
         open("results/Soay/comb_1Mb_map_Soay.txt",'w') as fout_comb:
        ...
        for i in range(26):
            ...
            for w_left in range(0,xlim,int(w_size_rec)):
                ...
                for i,o in enumerate(pseudo_obs):
                    print(chrom,w_left,w_right,'soay',i,o,file=fout_comb)
    ## 60K intervals map
    with open("results/Soay/SNP_array_map.txt",'w') as fout,\
